...
# ngrok Module
class ngrok:
    def __init__(self, TOKEN=None, USE_FREE_TOKEN=True, service=[['Service1', 80, 'tcp'], ['Service2', 8080, 'tcp']], region='us', dBug=[f"{HOME}/.ngrok2/ngrok.yml", 4040]):
        self.region = region
    ...
    def nameport(self, TOKEN, AUTO):
        if AUTO:
            ...
            if not 'your' in tokens.keys():
                ...
                print(r"Copy your ngrok authtoken from https://dashboard.ngrok.com/auth")
    ...
    def startWebUi(self, token, dport, nServer, region, btc, configPath, displayB, service, v):
        ...
        if v:
            clear_output()
            loadingAn(name="lds")
            textAn("Starting tunnel ngrok...", ty='twg')
...
class LocalhostRun:
    ...
    def keep_alive(self):
        ...
        for _ in range(2):
            self.connection=Popen(f"autossh -R 80:localhost:{self.port} {self.id}@ssh.localhost.run -o StrictHostKeyChecking=no -o ServerAliveInterval={self.interval} -o ServerAliveCountMax={self.retries}".split(), stdout=PIPE, stdin=PIPE, stderr=PIPE)
        ...
        if self.SERVICE == "localhost":
            ...
            if (proto=="tcp"):
                ...
                if v:
                    clear_output()
                    loadingAn(name="lds")
                    textAn("Starting tunnel localhost.run...", ty="twg")
            ...
            if v:
                ...
                loadingAn(name="lds")
                textAn("Starting tunnel argo tunnel...", ty="twg")
            ...
            if v:
                ...
                loadingAn(name="lds")
                textAn("Starting tunnel jprq...", ty="twg")
...
def findPackageR(id_repo, p_name, tag_name=False, all_=False):
    ...
    raise Exception("Not found or maybe the API has been changed!\n Try it again with different package name!")
...
def closePort(port):
    ...
    raise Exception("Unable to find the close port!")
